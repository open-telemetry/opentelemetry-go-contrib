...
// combine returns the higher-priority result. A proven cycle takes precedence
// over depth exhaustion, which takes precedence over a safe result.
func (r formatResult) combine(other formatResult) formatResult {
	if r == formatCycle || other == formatCycle {
		return formatCycle
	}
	if r == formatDepthLimit || other == formatDepthLimit {
...
// formattingRootNeedsCheck reports whether fmt can recursively traverse a root
// of type t deeply enough to require a value-level safety check.
...
// formattingTypeNeedsCheck reports whether values of t can contain a cycle or
// exhaust the traversal-depth bound under fmt's traversal rules.
func formattingTypeNeedsCheck(t reflect.Type) bool {
	return formattingTypeNeedsCheckAtDepth(t, nil, 0)
}

// formattingTypeNeedsCheckAtDepth walks the types fmt can recursively traverse,
// stopping at recursive types or the traversal-depth bound.
func formattingTypeNeedsCheckAtDepth(t reflect.Type, visited *formatTypeVisit, depth int) bool {
	...
	case reflect.Array, reflect.Slice:
		return formattingTypeNeedsCheckAtDepth(t.Elem(), visited, depth+1)
	case reflect.Map:
		return formattingTypeNeedsCheckAtDepth(t.Key(), visited, depth+1) ||
			formattingTypeNeedsCheckAtDepth(t.Elem(), visited, depth+1)
	case reflect.Struct:
		//nolint:modernize // Indexed reflection avoids iterator allocations on this path.
		for i := range t.NumField() {
			if formattingTypeNeedsCheckAtDepth(t.Field(i).Type, visited, depth+1) {
				return true
			}
		}
	}
...
// checkFormatValue mirrors fmt's aggregate traversal before fmt is called. A
// proven cycle takes precedence over depth exhaustion across sibling branches.
func checkFormatValue(v reflect.Value, depth int, visited *visitTracker) formatResult {
	...
	case reflect.Slice:
		...
		result := formatSafe
		for i := range v.Len() {
			result = result.combine(checkFormatValue(v.Index(i), depth+1, visited))
			if result == formatCycle {
				visited.pop(key)
				return formatCycle
			}
		}
	...
	case reflect.Map:
		...
		iter := v.MapRange()
		for iter.Next() {
			result = result.combine(checkFormatValue(iter.Key(), depth+1, visited))
			result = result.combine(checkFormatValue(iter.Value(), depth+1, visited))
			if result == formatCycle {
				visited.pop(key)
				return formatCycle
			}
	...
	case reflect.Struct:
		...
		//nolint:modernize // Indexed reflection avoids iterator allocations on this path.
		for i := range v.NumField() {
			result = result.combine(checkFormatValue(v.Field(i), depth+1, visited))
			if result == formatCycle {
				return formatCycle
			}
		}
