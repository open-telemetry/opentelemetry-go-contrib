...
// SpanProcessor is an sdktrace.SpanProcessor and http.Handler implementation
// that exposes zpages functionality for opentelemetry-go.
//
...
func newSampleStore(latencyBucketSize uint, errorBucketSize uint) *sampleStore {
	s := &sampleStore{
		...
		errors:  makeBucket(errorBucketSize),
	}
