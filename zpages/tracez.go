...
var _ http.HandlerFunc = (*SpanProcessor)(nil).ServeTracezHTTP

// ServeTracezHTTP implements the http.HandlerFunc and is capable of serving "tracez"
// HTTP requests.
func (ssm *SpanProcessor) ServeTracezHTTP(w http.ResponseWriter, r *http.Request) {
	r.ParseForm()
	...
	if err := summaryTableTemplate.Execute(w, ssm.getSummaryTableData()); err != nil {
		log.Printf("zpages: executing template: %v", err)
	}
	if spanName != "" {
		if err := tracesTableTemplate.Execute(w, ssm.getTraceTableData(spanName, spanType, spanSubtype)); err != nil {
			log.Printf("zpages: executing template: %v", err)
		}
	}
...
func (ssm *SpanProcessor) getTraceTableData(spanName string, spanType, latencyBucket int) traceTableData {
	var spans []sdktrace.ReadOnlySpan
	switch spanType {
	case 0: // active
		spans = ssm.activeSpans(spanName)
	case 1: // latency
		spans = ssm.spansByLatency(spanName, latencyBucket)
	case 2: // error
		spans = ssm.errorSpans(spanName)
	}
...
func (ssm *SpanProcessor) getSummaryTableData() summaryTableData {
	...
	for name, s := range ssm.spansPerMethod() {
