...
func TestChildSpanFromGlobalTracer(t *testing.T) {
	...
	handlerFunc := func(req *restful.Request, resp *restful.Response) {
		...
		require.True(t, ok)
		assert.Equal(t, "go.opentelemetry.io/contrib/instrumentation/github.com/emicklei/go-restful/otelrestful", mockTracer.Name)
...
func TestChildSpanFromCustomTracer(t *testing.T) {
	...
	container := restful.NewContainer()
	container.Filter(otelrestful.OTelFilter("my-service", otelrestful.WithTracer(tracer)))
	container.Add(ws)
...
func TestChildSpanNames(t *testing.T) {
	...
	container := restful.NewContainer()
	container.Filter(otelrestful.OTelFilter("foobar", otelrestful.WithTracer(tracer)))
	container.Add(ws)
...
func TestMultiFilters(t *testing.T) {
	...
	ws1.Route(ws1.GET("/{id}").
		Filter(otelrestful.OTelFilter("my-service", otelrestful.WithTracer(tracer1))).
		To(wrappedFunc("tracer1")))
	ws1.Route(ws1.GET("/{id}/books").
		Filter(otelrestful.OTelFilter("book-service", otelrestful.WithTracer(tracer2))).
		To(wrappedFunc("tracer2")))

	ws2 := &restful.WebService{}
	ws2.Path("/library")
	ws2.Filter(otelrestful.OTelFilter("library-service", otelrestful.WithTracer(tracer3)))
