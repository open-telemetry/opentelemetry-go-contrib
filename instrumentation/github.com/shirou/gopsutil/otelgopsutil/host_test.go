...
func TestHostCPU(t *testing.T) {
	...
	err := otelgopsutil.Start(
		otelgopsutil.WithMeterProvider(provider),
	)
	...
	processUser := getMetric(impl, "process.cpu.time", otelgopsutil.LabelCPUTimeUser[0])
	processSystem := getMetric(impl, "process.cpu.time", otelgopsutil.LabelCPUTimeSystem[0])

	hostUser := getMetric(impl, "system.cpu.time", otelgopsutil.LabelCPUTimeUser[0])
	hostSystem := getMetric(impl, "system.cpu.time", otelgopsutil.LabelCPUTimeSystem[0])
...
func TestHostMemory(t *testing.T) {
	...
	hostUsed := getMetric(impl, "system.memory.usage", otelgopsutil.LabelMemoryUsed[0])
	hostAvailable := getMetric(impl, "system.memory.usage", otelgopsutil.LabelMemoryAvailable[0])

	hostUsedUtil := getMetric(impl, "system.memory.utilization", otelgopsutil.LabelMemoryUsed[0])
	hostAvailableUtil := getMetric(impl, "system.memory.utilization", otelgopsutil.LabelMemoryAvailable[0])
...
func TestHostNetwork(t *testing.T) {
	...
	hostTransmit := getMetric(impl, "system.network.io", otelgopsutil.LabelNetworkTransmit[0])
	hostReceive := getMetric(impl, "system.network.io", otelgopsutil.LabelNetworkReceive[0])
