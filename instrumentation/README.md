...
- All instrumentation packages MUST define `ScopeName` constant with a value name matching the instrumentation package and use it when creating `Tracer` or `Meter`.
- All instrumentation packages MUST define `Version` function returning the version of the module containing the instrumentation and use it when creating `Tracer` or `Meter` .
