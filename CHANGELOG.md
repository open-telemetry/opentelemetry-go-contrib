...
- Add internal 1,000-edge depth and 10,000-unit work safety bounds to recursive container traversal in `go.opentelemetry.io/contrib/bridges/otellogr`, `go.opentelemetry.io/contrib/bridges/otellogrus`, `go.opentelemetry.io/contrib/bridges/otelslog`, and `go.opentelemetry.io/contrib/bridges/otelzap`, replacing detected recursive references with `"<cycle>"` and exhausted traversals with `"<depth-limit>"` or `"<work-limit>"` instead of overflowing the stack or exponentially expanding a compact graph. Non-reflexive map keys that cannot be retrieved are retained with invalid values instead of panicking. (#9542)
