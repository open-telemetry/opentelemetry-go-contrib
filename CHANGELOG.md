...
- Set `host.type` to only the machine type (e.g. `e2-medium`) instead of the full `projects/PROJECT_NUM/machineTypes/MACHINE_TYPE` metadata value in `go.opentelemetry.io/contrib/detectors/gcp`, following the semantic conventions.
