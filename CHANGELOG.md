...
- Add `go.opentelemetry.io/contrib/detectors/env` — a new resource detector for environment variable, ported from `processor/resourcedetectionprocessor/internal/env` in `opentelemetry-collector-contrib`. (#8956)
...
- Fix server spans using the client's address and port for `server.address` and `server.port` attributes in `go.opentelemetry.io/contrib/instrumentation/google.golang.org/grpc/otelgrpc`. (#9075)
