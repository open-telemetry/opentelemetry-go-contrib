...
- Add `error.type` to server spans and metrics in `otelhttp` when a request
  ends without a more specific status, preferring an observed response
  write error, request-body read error, or request-context error as the
  cause, and falling back to the status code otherwise. (#9358)
