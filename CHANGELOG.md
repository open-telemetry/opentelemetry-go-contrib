...
- Return canonical `http.Method*` constants from `standardizeHTTPMethod` in `go.opentelemetry.io/contrib/instrumentation/net/http/otelhttp`, `go.opentelemetry.io/contrib/instrumentation/net/http/httptrace/otelhttptrace`, `go.opentelemetry.io/contrib/instrumentation/github.com/gorilla/mux/otelmux`, `go.opentelemetry.io/contrib/instrumentation/github.com/gin-gonic/gin/otelgin`, `go.opentelemetry.io/contrib/instrumentation/github.com/labstack/echo/otelecho`, and `go.opentelemetry.io/contrib/instrumentation/github.com/emicklei/go-restful/otelrestful`, instead of the upper-cased input string. `net/http` derives `Request.Method` as a substring of the connection's read buffer via `strings.Cut`, and `strings.ToUpper` returns its input unchanged when no case conversion is needed, so recorded `http.request.method` attribute values could keep that entire read buffer reachable for as long as the span or metric data point referencing them was retained.
